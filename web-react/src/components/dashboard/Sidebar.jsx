...
function NavItem({ icon: Icon, label, active, badge, onClick }) {
  return (
    ...
    >
      ...
      <span className={`flex-1 text-left ${active ? '' : 'group-hover:text-blue-700'}`}>{label}</span>

      {badge && (
        <span className="px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 text-xs">{badge}</span>
      )}
...
export default function Sidebar({ activeSection, onSectionChange }) {
  ...
  return (
    <div className="w-72 h-screen bg-white border-r border-gray-200 flex flex-col relative overflow-hidden">
      <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/5 rounded-full blur-3xl" />
      <div className="absolute bottom-1/3 left-0 w-40 h-40 bg-blue-600/5 rounded-full blur-3xl" />

      <div className="p-6 relative z-10">
        <div className="flex items-center gap-3 mb-2">
          <div className="w-10 h-10 rounded-xl bg-white border border-blue-100 flex items-center justify-center shadow-md shadow-blue-200/20 overflow-hidden">
            <img src={usmlogo} alt="USM Logo" className="w-8 h-8 object-contain" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-gray-900">USM</h2>
              <span className="text-gray-300 text-lg font-light select-none">|</span>
              <span className="text-xs text-gray-500 whitespace-nowrap">Facultad del Usuario</span>
            </div>
            <p className="text-xs text-gray-500">Servicio Comunitario</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-gray-200 relative z-10">
        <div className="mb-4 p-4 rounded-xl bg-gradient-to-br from-blue-50 to-blue-100/50 border border-blue-200/50">
          <div className="flex items-center gap-2 mb-2">
            <div className="flex-1">
              <p className="text-xs text-blue-600 mb-1">Progreso Total</p>
              <p className="text-blue-400 italic">Sin datos</p>
            </div>
